...
| H19 | medium | harness-conformance | `agents/ticket-executor.md:48` | Executor artifact writes route through a symlink into the main checkout, which the harness blocks inside isolation worktrees — *documented as an open risk (needs a live wave-parallel dogfood on CC >= 2.1.218)* |
