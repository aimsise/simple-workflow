...
#
# Each test builds a temp git repo, seeds a specific .backlog/ / filesystem
...
#   E. Missing .backlog/ directory                        → no Active tickets line
#
...
# --- Fixture A: valid seeded phase-state.yaml in .backlog/active/ ---
setup_test_repo
...
if [ "$LAST_EXIT_CODE" -eq 0 ] && \
  ...
  echo -e "  ${GREEN}PASS${NC} Fixture A: valid .backlog/active ticket appears under 'Active tickets:'"
...
setup_test_repo
mkdir -p "$TEST_REPO/.backlog/product_backlog/001-bar"
write_valid_phase_state "$TEST_REPO/.backlog/product_backlog/001-bar" "create_ticket" "create_ticket" "in-progress"
run_hook "$HOOK" "" "$TEST_REPO"
TESTS_TOTAL=$((TESTS_TOTAL + 1))
CONTEXT=$(echo "$LAST_STDOUT" | jq -r '.additionalContext // ""')
if [ "$LAST_EXIT_CODE" -eq 0 ] && \
...
cat > "$TEST_REPO/.backlog/active/001-foo/phase-state.yaml" <<EOF
...
mkdir -p "$TEST_REPO/.backlog/active/001-foo"
write_valid_phase_state "$TEST_REPO/.backlog/active/001-foo" "scout" "create_ticket" "in-progress"
chmod 000 "$TEST_REPO/.backlog/active/001-foo/phase-state.yaml" 2>/dev/null || true

if [ ! -r "$TEST_REPO/.backlog/active/001-foo/phase-state.yaml" ]; then
  run_hook "$HOOK" "" "$TEST_REPO"
  TESTS_TOTAL=$((TESTS_TOTAL + 1))
  CONTEXT=$(echo "$LAST_STDOUT" | jq -r '.additionalContext // ""')
  if [ "$LAST_EXIT_CODE" -eq 0 ] && \
...
chmod 600 "$TEST_REPO/.backlog/active/001-foo/phase-state.yaml" 2>/dev/null || true
cleanup_test_repo

# --- Fixture E: missing .backlog/ → no Active tickets line ---
setup_test_repo
...
if [ "$LAST_EXIT_CODE" -eq 0 ] && \
  ...
  echo -e "  ${GREEN}PASS${NC} Fixture E: missing .backlog → original Branch + Changed files only, no 'Active tickets:' line"
  TESTS_PASSED=$((TESTS_PASSED + 1))
else
  echo -e "  ${RED}FAIL${NC} Fixture E: missing .backlog behaviour regressed"
  echo -e "       Exit code: $LAST_EXIT_CODE"
  echo -e "       Context:   $CONTEXT"
  TESTS_FAILED=$((TESTS_FAILED + 1))
fi
