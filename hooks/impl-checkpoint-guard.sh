...
if [ "$(echo "$INPUT" | jq -r '.hook_event_name // "Stop"' 2>/dev/null || echo Stop)" != "SubagentStop" ]; then
  ...
  if [ "$_sw_bg_inflight" -gt 0 ]; then
    echo "[IMPL-CHECKPOINT] standing down: $_sw_bg_inflight background agent task(s) in flight — the harness wakes the session when they return; allowing end_turn without counting a stall." >&2
    exit 0
  fi
fi
