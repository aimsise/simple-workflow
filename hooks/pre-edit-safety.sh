...
if ! command -v jq >/dev/null 2>&1; then
  case "${SW_SAFETY_JQ_MISSING_MODE:-metric-only}" in
    on)
      echo "[SAFETY-JQ-MISSING] pre-edit-safety: jq not found on PATH — failing closed (exit 2). Install jq (e.g. 'brew install jq') to run this guard." >&2
      exit 2 ;;
    off)
      exit 0 ;;
    metric-only|*)
      echo "[SAFETY-JQ-MISSING] metric-only: pre-edit-safety would fail closed (exit 2) — jq not found on PATH; allowing this call. Install jq, or set SW_SAFETY_JQ_MISSING_MODE=on to enforce." >&2
      exit 0 ;;
  esac
fi
      ...
      case "${SW_STATE_FIELD_GUARD_MODE:-metric-only}" in
        on)
          jq -nc --arg f "$_SFG_FIELD" \
            '{decision:"block", reason:("hook_owned_field_violation: " + $f + " is a hook-owned, append-only field written exclusively by the autopilot Stop / PreCompact / checkpoint hooks. Do not Edit it directly — let the hooks append. See docs/state-schema.md."), hookSpecificOutput:{hookEventName:"PreToolUse", permissionDecision:"deny", permissionDecisionReason:("hook_owned_field_violation: " + $f + " is a hook-owned, append-only field written exclusively by the autopilot Stop / PreCompact / checkpoint hooks. Do not Edit it directly — let the hooks append. See docs/state-schema.md.")}}'
          exit 0 ;;
        off) ;;
        metric-only|*)
          echo "[STATE-FIELD-GUARD] metric-only: would block hook_owned_field_violation: ${_SFG_FIELD} (Edit to $FILE_PATH); set SW_STATE_FIELD_GUARD_MODE=on to enforce. See docs/state-schema.md." >&2 ;;
      esac
    fi
    ;;
esac
