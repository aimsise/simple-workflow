...
Full argument signature: `/brief <what-to-build> [chain=on|off] [uc=on|off]` (default `chain=on`, `uc=on`). The `chain=on|off` form is canonical; `mode=auto|manual` is a deprecated legacy alias (`chain=on` ≡ `mode=auto`, `chain=off` ≡ `mode=manual`).
