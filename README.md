...
- [Claude Code](https://docs.anthropic.com/en/docs/claude-code) CLI **v2.1.232 or later**, installed and authenticated (the hooks and skills rely on hook-input fields, the `background:` skill field and the background-subagent default introduced up to that version; see Operational Notes)
...
### Wave-parallel runs: worktree base, remote, and Claude Code version

Wave-parallel execution (`parallel=on`, the default under `chain=on`) spawns each `ticket-executor` with the Agent tool's `isolation:"worktree"`. Since Claude Code v2.1.133 the platform bases such a worktree on `origin/<default-branch>` (`worktree.baseRef: "fresh"`, the default) and on the orchestrator's current `HEAD` only when you set `worktree.baseRef: "head"` in your settings or the repository has no remote. The executor re-points its worktree branch onto the run's local `ap-integration/<parent>` integration branch before it starts the pipeline, so later waves build on earlier waves' commits in either configuration; setting `"worktree": { "baseRef": "head" }` in `~/.claude/settings.json` merely turns that re-point into a no-op. The plugin's Stop / SubagentStop / PostToolUse hooks rely on hook-input fields that exist in Claude Code v2.1.218 or later (`agent_transcript_path`, `last_assistant_message`, the `background:` skill field); older hosts degrade to the transcript-scanning paths but are not tested.
...
- Designed for use with Claude Code CLI. IDE extensions (VS Code, JetBrains), the desktop app, and Claude Code on the web have no terminal multiplexer to receive the keystroke-injected `/compact`, so the auto-`/compact` between autopilot tickets is a silent no-op there (the run continues; context is bounded only by Claude Code's own auto-compaction), and hooks/plugin features may be limited.
- After an auto-compaction Claude Code re-attaches only the first ~5,000 tokens of each invoked skill (25,000 tokens across all skills). The long orchestrator skills (`/autopilot` ≈ 20k tokens, `/impl` ≈ 14k tokens) therefore rely on the `hooks/session-start.sh` re-injection of `/autopilot <slug>` to be re-read in full after a compaction; do not expect a compacted session to keep following the tail of a long skill without that re-injection.
...
- Sub-agents consume API tokens independently. The Generator (implementer) always runs on the `opus` alias (the current Opus generation; the `fable` alias selects a higher tier above it and is selectable per agent with `model: fable`), and the evaluator escalates from `sonnet` (the current Sonnet generation, 1M context on the API) to `opus` for critical/exhaustive work, so larger or higher-risk tickets may incur higher API costs. By default (ultracode orchestration is on; pass `uc=off` to revert to the single-evaluator Agent path), M+ tickets additionally run a 3-lens parallel evaluator panel (Sonnet at `thorough`, Opus at `exhaustive`), further increasing token cost.
