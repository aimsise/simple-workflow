...
|---|---|---|
| `researcher` agent (Agent tool) | Phase 1 Investigation — before drafting (bare mode always; brief mode unless a fresh `{ticket-dir}/investigation.md` already exists and is reused; findings mode skips because the findings doc IS the investigation) | No investigation findings; planner operates on model-internal assumptions rather than codebase evidence. Detected by missing researcher trace in skill invocation audit. **Reuse case (brief mode only)**: when `{ticket-dir}/investigation.md` is present and reused, the researcher invocation is intentionally skipped with no consequence — Phase 1 emits the same executive-summary + output-path schema sourced from the existing file, so the audit treats this as a contract-compliant skip rather than a bypass |
