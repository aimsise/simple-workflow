...
- **Available capabilities** (verbatim copy of the orchestrator's Pre-computed Context probes): the `Available user skills:` line AND the `Available MCP servers:` line MUST be inlined into the spawn prompt as-is. The subagent does not inherit the main-thread harness skill / MCP descriptions, so the orchestrator MUST serialise both probe outputs into the planner's prompt verbatim under a heading such as `## Available capabilities`. Empty probe outputs (`(none)`) are passed through unchanged — the planner needs to know whether the gap is "no capability found" vs "not yet probed".
