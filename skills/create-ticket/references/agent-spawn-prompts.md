...
Planner behaviour: draft every AC to satisfy Gates 1-10 on first pass; when the file-count axis and AC-count axis of Gate 5 disagree, include a short rationale in the ticket so the evaluator can apply the single-axis tiebreak rule. When the decomposer-returned skeleton carries `peer_set: true` (forwarded with `title, scope_summary, size, depends_on`), treat it as the upstream Gate 10 signal and assert the unified-convention AC over that peer set (re-deriving the peer set from the final Scope; the hint is advisory).
