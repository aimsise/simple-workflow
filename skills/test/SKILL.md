...
7. **Return contract**: the test-writer agent returns the structured envelope declared in `agents/test-writer.md` Context Conservation Protocol — under 500 tokens, with the fields **Status**: `success | partial | failed`, **Output**: `[test file path(s) created/modified]`, **Summary**: `[test count, pass/fail results]`, the REQUIRED **Advisory consultation**: field, and **Next Steps**: `[recommended actions]`. The orchestrator surfaces these fields to the user verbatim. The `**Advisory consultation**:` field (per `agents/test-writer.md` `### Consultation reporting format`) is mandatory: emit the literal `(none)` when the spawn prompt carried no `## Advisory capabilities (per ticket)` block or no entry's `Used by` column lists `test-writer`, otherwise emit one `- <Name>: invoked (<evidence>)` / `- <Name>: not invoked (<reason>)` bullet per applicable entry. Because `/test` is a declarative `context: fork` spawn (`context: fork` + `agent: test-writer` in the frontmatter makes the Claude Code platform run this skill body AS the test-writer's task prompt), there is no inline orchestrator turn after the fork to gate this field — the contract is carried by the test-writer agent body AND by this return-contract clause. `/test` has no orchestrator parent that re-spawns it (it is user-invoked or suggested by `/catchup`), so silent omission of the field surfaces as a Phase 6 audit-trail gap rather than a gated failure.
