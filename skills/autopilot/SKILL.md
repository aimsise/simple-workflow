...
##### Executor-routed per-ticket pipeline (`PARALLEL_MODE != off`)

Taken INSTEAD of the inline loop above when `PARALLEL_MODE != off`. The dependency graph, `PROCESSING_ORDER`, resume-skip semantics, and per-ticket terminal statuses are unchanged; the difference is that each ticket's pipeline runs inside a `ticket-executor` subagent (`agents/ticket-executor.md`) and the **main loop is the single writer** of `autopilot-state.yaml`.
