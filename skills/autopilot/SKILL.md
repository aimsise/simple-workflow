...
- "A precondition is unmet (missing git remote, untracked files, etc.), so I should ask the user to confirm before proceeding." -- WRONG. Treat the unmet precondition as `unexpected_error.action: stop`, emit the policy line + `## Stop Reason`, and exit.
