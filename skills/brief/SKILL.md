...
allowed-tools:
  ...
  - "Bash(ls:*)"
  - "Bash(date:*)"
argument-hint: "<what-to-build> [chain=on|off] [uc=on|off|metric-only] (legacy: mode=auto|manual)"
---
...
- Remove the parsed `chain=` AND `mode=` AND `uc=` tokens from the description; remaining text is `<what-to-build>`.
