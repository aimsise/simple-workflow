...
- **Metric-only / byte-identity.** The firewall defaults are `metric-only` (stderr only; no state-file write, no `.runtime_metrics` leak); all guards are autopilot-context-gated (a normal `git commit` outside an autopilot run is never touched), fail-open (exit 0; a block is decision JSON), and silent-exit when `jq` is absent. `SW_REVIEW_FIREWALL_MODE=off` / `SW_STATE_ADVANCE_GUARD_MODE=off` / `SW_PARALLEL_WORKTREE_PREFLIGHT_MODE=off` plus the existing `parallel=off` / `SW_PARALLEL_TICKETS_MODE=off` kill switches revert. **Owed:** a re-dogfood for the firewall mechanical path (AC-6) and the parallel end-to-end + SubagentStop-under-isolation-worktree (AC-8) gates the metric-only→on promotion (a follow-up).
