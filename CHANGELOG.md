...
**TL;DR.** A conformance pass against the September 2026 Claude Code harness (the `opus` / `sonnet` aliases resolve to the current generation and the `fable` alias selects a tier above `opus`; CLI 2.1.270), driven by a full audit of the plugin's skills, agents, hooks, tests and docs. Four places where the current harness no longer matched the plugin's assumptions are fixed on the default paths: the committed `eval-panel` Workflow script returned **nothing** to `/impl` Step 16 (a bare trailing expression is not a Workflow result), `context: fork` skills now run in the background by default so `/scout` read `/investigate`'s artifact before it existed, `isolation:"worktree"` executors branch from `origin/<default>` (not the orchestrator's HEAD) so wave-parallel cross-wave integration silently lost earlier waves on any remote-backed repository, and `SubagentStop` hooks received the parent transcript instead of the executor's. Two long-standing defects are fixed with them: the two checkpoint Stop guards could never reach their documented 3-attempt release (their own `runtime_metrics` write reset the counter every tick), and `parse_ticket_statuses` failed to re-import in every child bash under bash 5.2. Hook output shapes, the hooks-run-in-parallel contract, model-alias wording and several path / allowed-tools drifts are aligned with the current documentation. Non-breaking: every kill switch, argument and state-file field is unchanged; `parallel=off` / `uc=off` remain byte-identical to v10.0.0.

### Fixed

- **`eval-panel.mjs` returned no result (uc=on default path).** The script ended in a bare `merged;` expression; the Workflow runtime returns only a top-level `return` value (verified live), so `/impl` Step 16 never received the typed verdict. Now `return merged;`; `tests/test-eval-panel-merge.mjs` asserts the `return` and parses the script as an async Workflow body (a plain `node --check` cannot, so it is no longer the syntax gate).
- **Checkpoint Stop guards never released.** `hooks/impl-checkpoint-guard.sh` / `hooks/scout-checkpoint-guard.sh` wrote their block counter and THEN appended a `runtime_metrics` entry to the same state file, so the "state file newer than counter → reset" rule fired on every tick and the 3-attempt release was unreachable (reproduced: five ticks, counter pinned at 1). The metric is now recorded before the counter; `(REL-NAT)` in both guard suites pins `block,block,block,release`.
- **`SubagentStop` guards scanned the parent transcript.** The hook input carries `agent_transcript_path` (the stopping subagent's transcript, since CC 2.0.42) beside `transcript_path` (the session's); both guards now prefer `agent_transcript_path`, so executor-side enforcement under `parallel=on` reads the executor's turns. `(T-PAR-1b)` in both suites pins the preference.
- **`/investigate` and `/test` run in the background since CC 2.1.218.** Both `context: fork` skills declare `background: false` so the chain-calling `/scout` (and a direct caller) receives the artifact in the same turn.
- **Wave-parallel cross-wave base on remote-backed repositories.** Since CC 2.1.133 an isolation worktree is based on `origin/<default-branch>` (`worktree.baseRef: "fresh"`) and on the orchestrator's HEAD only under `worktree.baseRef: "head"` or with no remote — the no-remote case the T-008 spike validated. `agents/ticket-executor.md` gains step 1b: `git checkout -B <worktree-branch> ap-integration/<parent>` right after the state symlink (a no-op when the base already is the integrated tip; `failure_reason = integration_base_unavailable` when it cannot), and `skills/autopilot/SKILL.md` + `references/state-file.md` + README document the base-branch rule.
- **`parse_ticket_statuses` broke every child bash under bash 5.2.** The exported function kept its python heredoc inside an `if` condition, which bash 5.2 cannot re-import (`syntax error near unexpected token 'fi'` / `error importing function definition` on each child); the heredoc now sits on a simple `cmd <<'PY' && return 0` line like its siblings. `tests/test-hooks-lib.sh` pins zero import errors.
- **Workflow-path evaluator handoff was incomplete.** `/impl` Step 15 now passes `accept_set_triggered_on` (the deterministic `triggered-on=` list the script already read but never received), `plan_path`, `git_diff_shortstat`, `panel_directive`, `bound_capabilities`, `plan_compliance_hint` and `turn_budget`; the script inlines them with the Agent-path field names, defaults the per-lens report path into `ticket_dir` instead of the cwd root, and Step 16 maps `PASS_WITH_CAVEATS` (the value the merge emits). The merge drops `IN_PROGRESS` envelopes from the quorum and honours an envelope-level `FAIL_CRITICAL` (CRITICAL is never voted away); mirrored in `tests/test-eval-panel-merge.mjs` (28/28). `/impl` also falls back to the Agent path when the Workflow tool is unavailable, and `UC-FLOOR` no longer overrides a ticket's `verification_depth: off`.
- **Plugin-root paths and lookups.** `/audit` Step 4b and `/ship`'s review gate source `${CLAUDE_PLUGIN_ROOT}/hooks/lib/audit-coverage.sh` (the cwd-relative path only resolved inside the plugin repository); `ac-evaluator` / `ac-evaluator-hi` resolve `tautological-assertion-rules.md` against the plugin root; `planner` cites `skills/create-ticket/references/ac-quality-criteria.md` (the real path); `/ship`'s `ticket-dir=` lookup also resolves the nested `active/{parent-slug}/{NNN-slug}` layout; `/autopilot` Phase 1 step 3 honours `chain:` over the legacy `mode:` so the README's hand-edited `chain: on` rescue path works.
- **`hooks/pre-level1-guard.sh` auto-approved every Bash command** in the dev repository (`permissionDecision: allow` on the no-match path); it now emits no decision so the normal permission flow applies.
- **The wave-parallel state symlink could be committed and destroy the state tree.** `.gitignore`'s `.simple-workflow/` is a directory-only pattern that does not match the `.simple-workflow` SYMLINK each executor creates in its worktree; `/ship`'s autopilot "stage all" could commit it and the wave-boundary merge into the main checkout then replaced the real state directory with a self-referential link (reproduced by the audit). `/autopilot` Phase 2 init now writes `.simple-workflow` into the shared `.git/info/exclude`, the executor verifies it with `git check-ignore`, `/ship` stages with `git add . && git rm -r -q --cached --ignore-unmatch .simple-workflow` (an exclude pathspec is fatal inside the symlinked worktree and exits 1 with the ignored-path advice in the main checkout), and new-project setup adds the slash-less entry beside `.simple-workflow/` without repeating its header.
- **Stop hooks blocked the pause the harness needs for background subagents.** In interactive sessions subagents run in the background by default and the harness ends the turn until a task returns, reporting the in-flight tasks to Stop hooks as `background_tasks`. `autopilot-continue.sh` and both checkpoint guards now stand down (exit 0, no stall counted) while a subagent / workflow task is in flight on the main Stop (a `SubagentStop` still enforces), so a wave of background `ticket-executor`s is no longer force-continued into a thrash; `/autopilot`'s wave barrier is documented as "wait for every executor's completion notification".
- **`/autopilot` could not read its own env kill switches** (`SW_PARALLEL_TICKETS_MODE`, `SW_PARALLEL_MAX_CONCURRENCY`, `SW_AUTO_COMPACT_ON_SHIP_MODE`, …): a skill body has no access to the process environment. A pre-computed `Env kill switches` line now probes them; Argument Parsing (`SW_PARALLEL_TICKETS_MODE`, `SW_PARALLEL_MAX_CONCURRENCY`) and the auto-compact exception (`SW_AUTO_COMPACT_ON_SHIP_MODE`) read that line.
- **A non-mikefarah `yq` silently corrupted state parsing.** `hooks/lib/parse-state-file.sh` (and the `runtime-metrics` / `audit-coverage` helpers, `impl-checkpoint-guard`, `post-ship-state-auto-compact`) treated any `yq` on PATH as mikefarah v4; the python `yq` wrapper (`apt install yq`) accepts jq syntax and returns JSON-quoted or empty values, so the Stop-hook step parser saw no pending steps. `_psf_have yq` now checks `yq --version` for mikefarah v4 and falls through to the python / awk tiers otherwise (pinned by a fake-yq test).
- **Pre-computed context commands could abort a skill.** A `!`command`` that exits non-zero aborts the whole skill invocation; `/catchup`'s `git log -5` / `git diff --shortstat <default>` failed on a fresh or remote-less repository, and the bare git probes in `/audit`, `/autopilot`, `/impl`, `/investigate`, `/plan2doc`, `/refactor`, `/test` failed outside a git worktree. Every probe now degrades with `2>/dev/null || …`.
- **Tier-1 worktree cleanup used `git worktree remove --force`**, so the documented "dirty worktree is left on disk and logged" branch could never fire and uncommitted residue of a failed ticket was deleted; the per-ticket removal drops `--force` (the stale-sweep and the pre-flight probe keep it on clean-by-construction targets).
- **`detect-tautological-assertions.sh` R2 flagged meaningful bounds.** Any comparison against `0` / `Number.MIN_VALUE` was reported as vacuous, so `toBeGreaterThan(0)` (positivity) and `toBeLessThan(0)` (negativity) failed evaluator review; each operator is now paired only with its own vacuous extremum (fixture `g-positivity-not-vacuous`, in the static-rule suite's clean list).
- **`hooks/pre-level1-guard.sh` denied read-only commands** that merely mentioned the Level-1 test file names (`cat`, `grep`); it now matches execution forms only (`bash`/`sh`/`source`/`./` of `test-integration.sh` or `spike-claude-p.sh`; `tests/run-all.sh` is not matched because it only reaches Level 1 when `RUN_LEVEL1_TESTS=true` is already present) and runs under `set -euo pipefail`.
- **`parse_yaml_scalar` returned `True` for `parallel_mode: on` without mikefarah `yq`.** The python3 + PyYAML tier loads the YAML 1.1 scalars `on` / `off` / `yes` / `no` as booleans, so on any host with PyYAML but no mikefarah `yq` (a CI runner, a Linux box with the python `yq` wrapper) `resolve_parallel_mode` collapsed a `parallel_mode: on` run to `off` and every wave-aware hook silently took the serial path. The tier now re-reads the literal scalar for boolean results (`2.0d` in `tests/test-hooks-lib.sh`).
- **`CT-MODE-RM-3` was locale-dependent** — a Hiragana/Katakana/CJK character-class range false-tripped on any multibyte text in the C locale and errored (masked as PASS) under `C.UTF-8`; it now matches the UTF-8 byte ranges under `LC_ALL=C`.
- **`tests/test-autopilot-continue.sh` fed the hook invalid JSON.** The `run_autopilot_hook` helper defaulted its argument with `"${1:-{}}"`, which bash parses as default `{` plus a literal `}` appended to every caller-supplied payload; `jq` still emitted the first value before erroring on the trailing brace, so the existing cases passed by accident, and the new AC-BG case (whose `|| echo 0` fallback catches jq's non-zero exit) exposed it. The helper now defaults explicitly.

### Added

- `docs/harness-conformance-audit-2026-09.md` — the audit record: the reconstructed design perspective, the harness facts the plugin depended on, what v10.1.0 fixed, and every confirmed-but-deferred problem with its evidence (plus the unverified backlog), so the follow-up tickets can be cut from one tracked document.
- `tests/test-eval-panel-merge.sh` — a bash wrapper so `tests/run-all.sh` and CI actually run the Workflow merge unit test and the product-script contract (skips cleanly without node).

### Changed

- **Hook output shapes follow the current contract.** The PreToolUse guards (`pre-bash-contract-guard`, `pre-skill-contract-guard`, `pre-state-transition`, and the hook-owned-field deny in `pre-write-safety` / `pre-edit-safety`) emit `hookSpecificOutput.permissionDecision: deny` + `permissionDecisionReason` (the top-level `decision`/`reason` pair — deprecated for PreToolUse — is kept alongside for older hosts and the existing tests); `hooks/session-start.sh` emits `hookSpecificOutput.additionalContext` (top-level key kept). The three autopilot Stop hooks pass the harness's `last_assistant_message` (CC ≥ 2.1.47) to `last_turn_declares_policy_gate_stop`, which now takes it as an authoritative optional second argument and only falls back to the transcript scan when it is absent.
- **Hooks on one event run in parallel — the ordering rule is gone.** `CLAUDE.md` now states the documented contract ("All matching hooks run in parallel"; no ordering between top-level entries either) and requires order-independent hooks; `hooks/lib/runtime-metrics.sh` serialises its read-modify-write append with a portable `mkdir` lock (`<state_file>.lock`, bounded wait, stale-lock takeover) so the three Stop hooks appending in the same tick cannot lose an entry (8-way concurrency test in `tests/test-hooks-lib.sh`).
- **`allowed-tools` match the bodies.** `/autopilot` pre-approves `Agent` (the `ticket-executor` spawn), `/scout` `Glob` + `AskUserQuestion`, `/audit` `Write` + `Bash(source:*)`, `/ship` `Bash(rm|source|find|sort|head:*)`.
- **Honest wording.** The `.ship-commit-nonce` is described as a role-firewall sentinel that no model-writable state field can imitate — not as "non-forgeable" (any Bash write sink can create the file); `autopilot-policy-reference.md` states the real subagent model precedence (per-invocation > frontmatter > `CLAUDE_CODE_SUBAGENT_MODEL` > session) and dates its price ratio; the `design-oracles/aasc-accept-set` README no longer claims the mechanism is unshipped; `/autopilot` marks the concurrency-1 executor loop as the superseded Phase-1 intermediate.
- **Model / harness wording.** The "Agent JSONSchema rejects a per-spawn model" rationale is retired (the Agent tool accepts a per-invocation `model` that outranks frontmatter); the `ac-evaluator-hi` twin is kept as the canonical, auditable selection path. README / ARCHITECTURE state what `opus` / `sonnet` resolve to today and that `fable` sits above `opus`; README adds the wave-parallel base-branch / CC-version note, the no-multiplexer (web, desktop, IDE) auto-`/compact` limitation, and the post-compaction skill re-attach budget; `tests/README.md` lists every hook with its test file (`pre-level1-guard.sh` has none); `CONTRIBUTING.md` shows the full tree; README's minimum Claude Code version is 2.1.232 (the background-subagent default).

### Verification

- `bash tests/test-skill-contracts.sh` **918 / 918**; `bash tests/test-path-consistency.sh` **145 / 145**; `bash tests/run-all.sh` **ALL TEST SUITES PASSED** (39 suites, 2,252 assertions; incl. `test-impl-checkpoint-guard` 26/26, `test-scout-checkpoint-guard` 32/32, `test-hooks-lib` 180/180, `test-detect-policy-gate-stop` 21/21, `test-autopilot-continue` 66/66, `test-ac-evaluator-static-rules` 22/22 across all 7 fixtures, the new `test-eval-panel-merge.sh` wrapper 1/1); `node tests/test-eval-panel-merge.mjs` **28 / 28**; `shellcheck --severity=warning` clean on `hooks/*.sh hooks/lib/*.sh skills/impl/lib/*.sh tests/helpers/*.sh` and every `tests/test-*.sh` except the 11.7k-line `test-skill-contracts.sh` (touched only in one comment; the linter exhausts the sandbox process memory on it); `claude plugin validate --strict .` passes; twin byte-identity (`CT-EV-MODEL`) preserved. All suites run with mikefarah `yq` v4 on PATH (as on the GitHub `ubuntu-latest` runner); with no `yq` at all the hooks take their python3 / awk tiers and every suite still passes except two pre-existing test-side assertions that call `yq` themselves (`test-state-parsers.sh` AC-8c..j, `test-post-ship-state-auto-compact.sh` AC-3).
- **Live probes on CC 2.1.270.** Two zero-agent Workflow scripts — one ending in a trailing expression, one in `return` — proved only the `return` form yields a result. The impl checkpoint guard was run five consecutive ticks against the `NEEDS_AUDIT` fixture before the fix: `decision=block` with the counter pinned at 1 on every tick, no release.
- **Not re-dogfooded end-to-end** (no live autopilot run in this pass): the `isolation:"worktree"` Edit/Write check may refuse artifact writes that go through the executor's `.simple-workflow` symlink into the main checkout (the harness blocks writes "that target a path in the main checkout"; whether it resolves symlinks is undocumented) — `parallel=off` is the escape hatch until a wave-parallel dogfood on CC ≥ 2.1.232 confirms the path.

## [10.0.0] — 2026-06-28

**TL;DR.** `/brief` no longer auto-builds by default. A bare `/brief <idea>` now defaults to **`chain=off`**: it writes the brief (plus its `autopilot-policy.yaml` rescue file) and **stops** with manual next-steps, instead of chaining straight through `/create-ticket` → `/autopilot` to commits and PRs. The autonomous build pipeline — including v9.0.0 wave-parallel execution — is now an explicit opt-in: pass **`chain=on`** (`/brief <idea> chain=on`) to restore the prior idea-to-PR behavior. Under `chain=on`, `uc` and `parallel` still default `on` (unchanged). The `mode=auto|manual` legacy alias is unchanged and still accepted; only the *default* flips.

### BREAKING CHANGES

- **Bare `/brief <idea>` now defaults to `chain=off` (was `chain=on`).** Previously an argument-less `/brief` chained into `/create-ticket` and `/autopilot`, running the full autonomous pipeline — and, since v9.0.0, spawning N wave-parallel `ticket-executor` worktrees that commit and open PRs — with no further input. That implicit, hard-to-reverse, outward-facing behavior was a poor fit for an invocation that reads as "write a brief," and v9.0.0 widened its blast radius (N× concurrent worktrees + token spend). The default is now `chain=off`: `/brief <idea>` writes the brief + policy and stops with the Step 3 manual guidance. **Migration:** to restore the prior auto-build behavior, pass `chain=on` explicitly — `/brief <idea> chain=on`. Everything downstream of an explicit `chain=on` is byte-identical to v9.0.x (same `/create-ticket` → `/autopilot` handoff, same chained `uc=on` / `parallel=on` defaults). The `mode=auto|manual` alias is retained and unchanged — `mode=auto` ≡ `chain=on` still opts into the full pipeline. Read-time resolution of pre-v6.0.0 key-less briefs is unchanged (still treated as `auto`); only the *write-time* default for a bare invocation flips.

### Changed

- `skills/brief/SKILL.md` — Argument Parsing, the mode-independence guard, the frontmatter-derivation notes, and the Finalization Step 2 gating now document `chain=off` as the both-keys-omitted default (the `uc`/`parallel` resolution and every `mode=` alias literal are preserved verbatim). A bare invocation deterministically takes the Step 3 manual path (no `auto-kick.yaml` written, `next_recommended_auto: ""`), so the Stop-hook auto-continue path does not fire.
- `README.md` — the opening pitch, the full-argument signature, the ultracode / wave-parallel default notes, the mode table, the execution-chains block, and the hero-diagram caption now present `chain=off` as the default and `chain=on` as the opt-in for full automation. The detailed flow diagram (`docs/brief-chain-flow*.png`, regenerated) relabels its entry box to `/brief "<idea>" chain=on (opt-in)`.
- `create-ticket` is intentionally unchanged: running `/create-ticket` on a manual (`chain=off`) brief still prints `next_recommended_auto: /autopilot <slug>` for N>1, and running `/autopilot` on a manual brief lands on the existing (v8.4.2) graceful hard-stop (`reason=brief_mode_manual` + re-propagation directive). This brief → `/create-ticket` → `/autopilot` manual path becomes more common under the new default but is non-looping and self-recovering, so no contract change was warranted.

### Verification

- `tests/test-skill-contracts.sh`: **915 / 915** (was 914 / 914; +1 = `CT-MODE-18`, which pins the `chain=off` default at the brief Argument Parsing source + the Step 2 bare-skip clause + the README signature, and drift-guards — via a negative clause — against the pre-v10.0.0 `chain=on` default silently returning). The `mode=`-alias contract tests `CT-MODE-1..9` and the parallel-default tests `CT-PARALLEL-1` / `CT-PARALLEL-7` stay green (no alias removal, no change to `uc`/`parallel` resolution under `chain=on`).
- `tests/test-brief-lightening.sh`: 12 / 12. `tests/test-path-consistency.sh`: 145 / 145. `tests/run-all.sh`: ALL PASSED. `shellcheck --severity=warning` (matching CI) clean; `CT-DECONTAM-1` clean; `ac-evaluator` / `ac-evaluator-hi` twin byte-identity preserved (no agent edits this release).
...
- **Advisory Capabilities Recommending semantics (v8.0.0+
  ...
  (`test_simple_workflow35`) confirmed
