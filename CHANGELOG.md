...
Hardens the `/impl` orchestration loop against two recurring manual-run footguns surfaced while shipping v6.2.2: the Generator can silently skip files declared in the plan's Affected-files table, and a stray unprotected `grep -c` under `tests/` can kill a `set -euo pipefail` test section mid-run when a count is zero. Both classes of failure are now caught before they reach the AC Evaluator. No existing skill behaviour changes when plans declare no Affected-files section or when every declared file is in the diff. Non-breaking, no migration required.
...
- `tests/test-skill-contracts.sh` gains a new `Cat Q: Test-suite shell hygiene` category with two assertions. `CT-MODE-GREP-C-1` scans every `grep -c` invocation under `tests/` and fails if any is unprotected, where "protected" means an inline `|| true` / `|| echo 0` / `|| count=0` / `|| return 0` / `|| exit N` / `|| count_matches` guard, a surrounding `set +e` ... `set -e` block, a comment line, or an `echo` / `printf` documentation string. `tests/test-helper.sh` is exempt because it owns the `count_matches` primitive that legitimately wraps `grep -c`. `CT-MODE-GREP-C-2` asserts `agents/ac-evaluator.md` `maxTurns >= 30` so a future regression that lowers the cap below the v6.2.2 floor (60) cannot silently undo the AC-budget headroom.

### Fixed

- Seven previously-unprotected `grep -c` callsites under `tests/` migrated to either `count_matches` (in files that already source `tests/test-helper.sh`) or an inline `|| echo 0` guard (in files that do not): `tests/test-hooks-lib.sh:355`, `tests/test-path-consistency.sh:224,788`, `tests/test-per-phase-metrics.sh:321,341`, `tests/test-session-start.sh:272,347`, and `tests/test-skill-contracts.sh:3420`. Each was a latent footgun: a zero match would exit the pipeline with status 1, and `set -euo pipefail` would terminate the script before the test framework could record a clean FAIL. The migration is functionally identical when matches exist (count is unchanged); when matches are zero the count is now `0` and the surrounding assertion records a normal FAIL instead of a hard exit.
