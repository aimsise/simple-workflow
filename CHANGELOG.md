...
### Added
- `ac-evaluator` tautological assertion static detector (PX-07). New canonical rule file `skills/impl/references/tautological-assertion-rules.md` defines three deterministic anti-patterns — **R1** (reference equality of the same symbol), **R2** (vacuous numeric boundary against a literal extremum such as `0`, `Number.MAX_VALUE`, `-Infinity`), and **R3** (constant-only boolean assertions including `expect(true).toBe(true)` and short-circuit constants like `X || true`). Each rule carries a BAD / GOOD example pair, a hint-comment exemption (`// intentional reference equality test`) for legitimate reference-identity tests, and a `## Limitations` section documenting first-stage grep-based scope (variable resolution, type inference, cross-file analysis, and AST-level algebraic simplification remain out of scope). `agents/ac-evaluator.md` gains a `## Tautological Assertion Static Rules` block instructing the agent to load the rules file every round and surface violations as `R<N>: <file>:<line> — <excerpt>` in the Feedback field. A reference shell detector at `skills/impl/lib/detect-tautological-assertions.sh` and six fixtures under `tests/fixtures/tautological-assertions/` back the contract; `tests/test-ac-evaluator-static-rules.sh` exercises all six scenarios (three FAIL fixtures with rule IDs, three PASS fixtures including the hint-exempt and non-constant-bound cases). No env-var bypass and no warning-only mode.
- `hooks/lib/forbidden-rationale-patterns.sh` and `hooks/lib/parse-state-file.sh` (PX-01): two shared helper scripts under the new `hooks/lib/` directory, sourced by future PreToolUse / PostToolUse guards and by `tests/test-skill-contracts.sh`. `forbidden-rationale-patterns.sh` exports the canonical `FORBIDDEN_RATIONALE_PATTERNS` Bash array (10 POSIX-extended regexes covering `context.*budget`, `context.*pressure`, `context.*exhaust(ed|ion)?`, `context.*occupancy`, `context.*window.*press`, `token.*budget`, `running out.*context`, `release valve`, `pressure relief`, `pragmatic shortcut`) so every guard checks the same rationale list. `parse-state-file.sh` exposes four functions — `is_autopilot_context`, `parse_phase_status`, `parse_ticket_statuses`, `find_state_file` — backed by a `yq` → `python3 + PyYAML` → `awk` fall-through that matches the graceful-degrade contract documented in `CLAUDE.md ## Dependencies`. Neither helper introduces an environment-variable bypass.
...
- `tests/test-skill-contracts.sh` Category CP (PX-01 AC #1..#4): four new assertions — `CT-MODE-CP-1` (Manual Bash Fallback bullet list mentions context budget / window / pressure / exhaustion / occupancy), `CT-MODE-CP-2` (`## Context-Pressure Response Paths` heading sits immediately above `## Stop Reason`), `CT-MODE-CP-3` (the new section body grep-matches `pre-compact-save.sh`, `[RESUME] Skipping`, `unexpected_error.action: stop`, and `AskUserQuestion`), `CT-MODE-CP-4` (`stop-reason-taxonomy.md` carries the literal phrase `auto compact is normal operation`).
- `hooks/pre-bash-contract-guard.sh` (PX-02a): new `PreToolUse:Bash` hook that blocks Manual Bash Fallback contract violations at the moment a Bash call is about to fire, registered as a second entry under the existing `Bash` matcher in `hooks/hooks.json` so it runs alongside `pre-bash-safety.sh`. The guard is a no-op outside an autopilot tree (`is_autopilot_context()` from `hooks/lib/parse-state-file.sh`); inside the tree it (1) iterates the imported `FORBIDDEN_RATIONALE_PATTERNS` array (no local pattern duplication) and rejects appends to `manual_bash_fallbacks[]` whose `reason` text matches a forbidden rationale (`context_budget_fallback`), and (2) rejects direct `git commit` invocations when no per-ticket `phase-state.yaml` carries `phases.ship.status: in_progress` (`unauthorized_ship_inline`, the `/ship` Skill bypass case). No environment-variable bypass (no `SKIP_*` / `BYPASS_*` / `FORCE_*`) and no rate / threshold concept. The `skills/autopilot/SKILL.md` Manual Bash Fallback Discipline section gains a runtime-enforcement paragraph naming the hook. `tests/test-pre-bash-contract-guard.sh` exercises all five PX-02a AC #4 scenarios (a–e) under self-contained tempdir fixtures so the suite runs without `yq` installed.
- `tests/test-skill-contracts.sh` Phase B (PX-02b): post-hoc audit that scans `manual_bash_fallbacks[].reason` text in fixture YAML files for the canonical `FORBIDDEN_RATIONALE_PATTERNS` regex list sourced from `hooks/lib/forbidden-rationale-patterns.sh` (no local pattern duplication). The Phase B helper `phase_b_run_case` records an `expected: detect-forbidden` outcome for the FAIL fixture and `expected: clean` for the PASS fixture, treating each as a passing case when the actual outcome matches the expectation; one matching reason is sufficient to flag a fixture (no acceptable-rate threshold). Fixtures: `tests/fixtures/manual-bash-fallbacks-samples/with-context-budget-reason.yaml` (FAIL, exercises `context.*budget` and `token.*budget`) and `tests/fixtures/manual-bash-fallbacks-samples/clean-anomaly-reasons.yaml` (PASS, three legitimate subagent-failure / aliased-binary-bypass / transient-network rationales). Phase B is the run-after counterpart to PX-02a's PreToolUse:Bash guard: it flags any rationale that slipped past the runtime block (e.g. autopilot-context detection false-negative or hook outage). Detection is restricted to the `reason` field — the `invocation_method == manual-bash` value itself remains a legitimate state for anomaly recovery.
- `tests/test-precompact-end-to-end.sh` (PX-06): end-to-end fixture that forces the PreCompact hook (`hooks/pre-compact-save.sh`) to fire by piping a Claude Code-shaped JSON payload (`session_id`, `cache_creation_input_tokens`, `cache_read_input_tokens`, `input_tokens`) on stdin against a `mktemp -d` repo skeleton, then asserts that `runtime_metrics:` gains a `boundary: session_compaction` entry. Four scenarios cover the PX-03 hook-discovery extension and its NAC #7 guard: (a) `briefs/active/` only → append, (b) `briefs/done/` with all steps `completed` → append, (c) `briefs/done/` with a pending step → no append (state file byte-identical), (d) no state file in any of the three lookup roots → graceful exit 0 with no `runtime_metrics:` write. Verification prefers `yq '.runtime_metrics[] | select(.boundary == "session_compaction")'` and degrades to a grep-based check when `yq` is missing, mirroring the hook's own `yq` → `python3 + PyYAML` → pure-shell fall-through. The fixture isolates every scenario inside its own tmp dir, never touches the host project's `.simple-workflow/`, and cleans up via `trap cleanup EXIT`.
- `hooks/pre-state-transition.sh` (PX-04): new `PreToolUse:Write` and `PreToolUse:Edit` guard that intercepts state-file writes targeting `autopilot-state.yaml` or `phase-state.yaml` inside an autopilot context and blocks unauthorized `status: skipped` transitions before the file is mutated. Registered as a second entry under each of the existing `Write` / `Edit` matchers in `hooks/hooks.json` so it runs alongside `pre-write-safety.sh` / `pre-edit-safety.sh`; out-of-scope file basenames pass through silently. The hook is a no-op outside an autopilot tree (`is_autopilot_context()` from `hooks/lib/parse-state-file.sh`); inside the tree it parses the proposed YAML body and rejects (1) `status: skipped` writes when at least one sibling ticket is `pending` / `in_progress` and the affected ticket carries no ticket-level `override_skip: true` and no `dependency_failed` / `dependency_skipped` cascade marker (`unauthorized_skip_with_active_siblings`), and (2) `status: skipped` writes whose `skip_reason` matches any pattern in the imported `FORBIDDEN_RATIONALE_PATTERNS` array (`hooks/lib/forbidden-rationale-patterns.sh`) even when `override_skip: true` is set (`unauthorized_skip_with_forbidden_rationale`) — this closes the override-as-context-budget-bypass second escape route. The override flag must sit at the same indentation as the ticket's `status:` line; a top-level `override_skip:` or a commented form does not count, so a misplaced override cannot smuggle a skip through. No environment-variable bypass (no `$SKIP_*` / `$BYPASS_*` / `$FORCE_*`), no `AskUserQuestion` escalation path, no `PreToolUse:Read` registration. `skills/autopilot/SKILL.md` Per-ticket pipeline / Dependency check section gains a `MUST NOT` skip-transition invariant clause naming the two contracted exceptions (dependency cascade, ticket-level `override_skip` with non-forbidden rationale). `skills/create-ticket/references/phase-state-schema.md` adds Section 8 documenting the `override_skip` field schema and the abuse-prevention rule that a forbidden rationale invalidates an override regardless of structural placement, citing `hooks/lib/forbidden-rationale-patterns.sh` as the single source of truth without duplicating the pattern list. `tests/test-state-transition-guard.sh` exercises all six PX-04 AC #6 scenarios (a–f) plus dependency-cascade allow, initial-pending allow, Edit-form coverage, out-of-scope basename allow, outside-autopilot allow, and a static no-escape-hatch check, all under self-contained tempdir fixtures.
- `hooks/post-phase-checkpoint.sh` (PX-05): new `PostToolUse:Write` and `PostToolUse:Edit` hook that appends `boundary: phase_complete` / `phase_failed` / `phase_skipped` entries to the parent `autopilot-state.yaml.runtime_metrics` whenever a `phase-state.yaml` write transitions one of `phases.<name>.status` (with `<name>` in `scout` / `impl` / `audit` / `tune` / `ship`) to one of those three terminal values. Registered as two new entries under the existing `PostToolUse` block in `hooks/hooks.json` (matchers `Write` and `Edit`) so a clean six-ticket run records up to thirty per-phase entries (6 tickets × 5 phases) instead of the single session-level entry that the existing Stop / PreCompact writers emit per session — Plan 07 dynamic phase shrinking needs that observation density. The hook is a silent no-op outside an autopilot tree (`is_autopilot_context()` from `hooks/lib/parse-state-file.sh`); inside the tree it reads the post-Write phase-state.yaml via `parse_phase_status()`, resolves the parent `autopilot-state.yaml` via `find_state_file()` (which inherits the PX-03 `briefs/done/` lookup root), and appends one entry per terminal status. Idempotency is enforced by a full-array `(ticket_id, phase, boundary)` triple check — a repeated Write of the same status (e.g. `completed` → `completed`) does not produce a second entry, and the check scans the entire `runtime_metrics:` array rather than a recent-N window so duplicates are detected even after the list grows past 30 entries. Append uses `yq` → `python3 + PyYAML` → pure-shell heredoc graceful degrade, mirroring `autopilot-continue.sh` / `pre-compact-save.sh`. Per-phase entries carry `stop_reason: null` and best-effort `cache_*` / `input_tokens` / `consecutive_stop_blocks` (defaulted to `null` since the PostToolUse:Write payload does not include token usage); the existing `session_end` / `session_compaction` writers and their entry shape are unchanged. No environment-variable bypass (no `SKIP_*` / `BYPASS_*` / `FORCE_*`); emit failures warn to stderr (`[per-phase-emit] WARN:`) and exit 0 so autopilot progression is never halted by an observability outage. `skills/autopilot/references/stop-reason-taxonomy.md` adds the three new boundary literals to the `boundary` table, a per-phase-entries field reference (extra `ticket_id` and `phase` columns, the `stop_reason: null` invariant, and the array-wide idempotency rule), and a clarification that for `phase_complete` / `phase_failed` / `phase_skipped` the `stop_reason` field is always `null`. `tests/test-per-phase-metrics.sh` exercises all seven PX-05 AC #3 scenarios (a–g) plus the AC #5 30-entry simulation, with self-contained tempdir fixtures under `tests/fixtures/per-phase-metrics-samples/` (`autopilot-state-base.yaml` and `phase-state-pending.yaml` templates) and a sanity check on the static AC #1 / AC #2 / AC #6 / NAC #10 grep contracts.

### Changed
- `skills/autopilot/SKILL.md` `## Manual Bash Fallback Discipline` (PX-01): added a one-paragraph clarification immediately under the canonical Manual Bash Fallback definition stating that context window / context budget pressure is **NEVER** an anomaly, and a third bullet under `**MUST NOT treat as Manual Bash Fallback**:` enumerating the forbidden context-pressure rationales (`context window`, `context budget`, `context pressure`, `context exhausted`, `context occupancy`, `token budget`) and pointing readers at `hooks/lib/forbidden-rationale-patterns.sh`. The canonical definition line ("orchestrator-level `Bash` call ... subagent could not handle") is preserved verbatim.
- `skills/autopilot/SKILL.md` (PX-01): inserted a new `## Context-Pressure Response Paths` section immediately above `## Stop Reason`. The section enumerates two canonical responses — **(a)** accept auto-compaction (PreCompact hook + `[RESUME] Skipping {logical_id}: already completed` resume path) and **(b)** stop via `unexpected_error.action: stop` (policy-gate stop emitting `tag: policy_gate_stop`) — and explicitly forbids a third path (no `AskUserQuestion`, no inline-equivalent fallback, no Skill bypass to "save tokens"). All bullet predicates use `MUST NOT` / `NEVER` verbs (no `should` / `may` / `recommended` softening).
