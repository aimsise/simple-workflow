...
Your `## Result` envelope above — including the REQUIRED `**Advisory consultation**:` field — is your most important deliverable: the spawner reads it to decide what happens next, and your detailed findings already live in the output file written via `Write`/`Edit`. If you sense you are approaching your `maxTurns` ceiling while still gathering evidence, STOP and emit the envelope as `partial` with what you have, rather than risk a truncated turn that returns no envelope at all (which would strand both your summary and the Advisory audit trail for any capability you invoked). Detailed findings are already on disk; the envelope is the only thing the spawner consumes directly, so it must always be emitted. The `**Advisory consultation**:` field is REQUIRED on every exit path (`success` / `partial` / `failed`) — record any Advisory capability you invoked as `invoked (<evidence>)` even when bailing early.
...
- **Declarative spawner, standalone** (`/investigate <topic>` typed directly) cannot gate via an inline orchestrator step because the spawn is automated by the Claude Code platform via `context: fork` + `agent: researcher` in the frontmatter — there is no orchestrator turn after the fork. The contract is enforced by this agent body plus the `/investigate` skill-body return contract (`skills/investigate/SKILL.md` Step 6): silent omission makes the field absent in the returned executive summary, a Phase 6 audit-trail gap visible to any downstream consumer of `/investigate`'s output.
- **`/scout`-mediated path** (when `/scout` invokes `/investigate` via the Skill tool) IS gated at the caller: `/scout` resumes after `/investigate` returns and runs `/scout` Step 4a — present → `[ADVISORY-CONSULT] scout researcher present`; absent → `[PIPELINE] scout: ADVISORY-MISSING (agent=researcher)` surfaced in the research summary. This is surface-don't-fail (the round is not failed) because `investigation.md` is already on disk and `/plan2doc` reads that file, not the return summary; the gate exists to make the audit-trail gap visible, not to break the pipeline.
