...
- `MAIN_REPO` — (`PARALLEL_MODE == on`, T-008) the absolute path of the main checkout (`main_checkout_root`), used as the TARGET of the `.simple-workflow` symlink you self-create as your first step (`ln -s <MAIN_REPO>/.simple-workflow .simple-workflow`). The platform spawned you with `isolation:"worktree"`, so your cwd is already a per-ticket worktree under `<MAIN_REPO>/.claude/worktrees/agent-<id>` (branch `worktree-agent-<id>`; the platform bases it on `origin/<default-branch>` under the default `worktree.baseRef: "fresh"` and on the orchestrator's HEAD = the `ap-integration/<parent>` tip only under `worktree.baseRef: "head"` or with no remote — your step 1b re-points it onto `ap-integration/<parent>` either way) — the pipeline's product-source edits + the ship commit land on that branch, isolated from siblings. You do NOT receive or enter a `WORKTREE_PATH`. Absent on the serial / concurrency-1 path (no worktree = main checkout).
...
Under the wave scheduler you are spawned with `isolation:"worktree"`, so the platform has ALREADY created your per-ticket worktree (under `<MAIN_REPO>/.claude/worktrees/agent-<id>`, on branch `worktree-agent-<id>`; its base is `origin/<default-branch>` under the platform default `worktree.baseRef: "fresh"`, or the orchestrator's current HEAD = the `ap-integration/<parent>` tip under `worktree.baseRef: "head"` / with no remote — step 1b below makes the base the integrated tip in both cases) and your cwd is already that worktree. Run the pipeline there:

1. **Self-create the `.simple-workflow` state symlink FIRST, before any pipeline skill.** `ln -s <MAIN_REPO>/.simple-workflow .simple-workflow` — the absolute `MAIN_REPO` from your spawn prompt is the link TARGET (an absolute target is load-bearing: a relative target would resolve inside the worktree, not the shared tree). The gitignored `.simple-workflow/` tree is ABSENT in a fresh worktree, so this symlink is what makes every relative `.simple-workflow/...` path the pipeline uses resolve to the SHARED main checkout. Do NOT call `EnterWorktree` — the `isolation:"worktree"` spawn already placed you in the worktree (you do not enter it by path). Never create a `.worktreeinclude` to copy state (W-8 — a copy would diverge; the symlink shares ONE inode). **Never stage or commit the symlink**: the orchestrator lists `.simple-workflow` in `<MAIN_REPO>/.git/info/exclude` at Phase 2 init (the `.gitignore` pattern `.simple-workflow/` is directory-only and does not match a symlink); verify with `git check-ignore -q .simple-workflow` and, if it is NOT ignored, append the entry yourself (`printf '.simple-workflow\n' >> <MAIN_REPO>/.git/info/exclude`) before any pipeline skill runs — a committed symlink merged into the main checkout would replace the real state tree with a self-referential link.

1b. **Re-point your worktree branch onto the integrated tip — after the symlink, BEFORE any pipeline skill.** Run `git checkout -B "$(git branch --show-current)" ap-integration/<parent_slug>` (the `parent_slug` from your spawn prompt). The platform bases an isolation worktree on `origin/<default-branch>` by default (`worktree.baseRef: "fresh"`, Claude Code ≥ 2.1.133) and on the orchestrator's HEAD only under `worktree.baseRef: "head"` or when the repository has no remote; `ap-integration/<parent>` is the orchestrator's integrated tip that every wave must build on, so this re-point is what lets a later-wave ticket see earlier waves' committed changes. It is a no-op when the base already IS that tip, it touches only your own fresh `worktree-agent-<id>` branch (which has no commits of its own yet), it never redirects git at the main checkout (the branch is shared through the common `.git`, not checked out there — the orchestrator is on it in the main checkout, which is why you re-point rather than check it out), and the untracked `.simple-workflow` symlink from step 1 survives the checkout. If the command fails (the integration branch is missing or the checkout is refused), set `status = failed`, `failure_reason = integration_base_unavailable`, and emit the envelope — do NOT run the pipeline on an unknown base.
